...
func NewCalculatorProcessor(handler Calculator) *CalculatorProcessor {
	...
	self10 := &CalculatorProcessor{handler: handler, processorMap: make(map[string]thrift.TProcessorFunction)}
	self10.processorMap["ping"] = &calculatorProcessorPing{handler: handler}
	self10.processorMap["add"] = &calculatorProcessorAdd{handler: handler}
	self10.processorMap["calculate"] = &calculatorProcessorCalculate{handler: handler}
	self10.processorMap["zip"] = &calculatorProcessorZip{handler: handler}
	return self10
}
...
func (p *calculatorProcessorPing) Process(ctx context.Context, seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	if err2 := p.handler.Ping(ctx); err2 != nil {
		...
		_exc13 := thrift.NewTApplicationException(thrift.INTERNAL_ERROR, "Internal error processing ping: "+err2.Error())
...
func (p *calculatorProcessorCalculate) Process(ctx context.Context, seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	if retval, err2 := p.handler.Calculate(ctx, args.Logid, args.W); err2 != nil {
		...
		default:
			...
			_exc17 := thrift.NewTApplicationException(thrift.INTERNAL_ERROR, "Internal error processing calculate: "+err2.Error())
